...
function tournamentWinner(competitions, results) {
  ...
  const teams = {};
  // while assigning, set value to either 0 or 1 for each key
  for (let i = 0; i < results.length; i++) {
    const homeTeam = competitions[i][0];
    const awayTeam = competitions[i][1];
    const homeScore = results[i] === 0 ? 0 : 1;
    const awayScore = results[i] === 1 ? 0 : 1;
    teams[homeTeam] = teams[homeTeam] ? teams[homeTeam] += homeScore : homeScore;
    teams[awayTeam] = teams[awayTeam] ? teams[awayTeam] += awayScore : awayScore;
  }

  const scores = Object.values(teams);
  const max = Math.max(...scores);
  const winner = Object.keys(teams).find(key => teams[key] === max);

  return { winner, scores: teams };
}
